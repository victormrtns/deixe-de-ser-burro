...
export function PublicBookPage() {
  ...
  return (
    <PublicReadingShell>
      {books.isLoading || articles.isLoading ? <p className="public-status" role="status">Abrindo o caderno…</p> : null}
      {books.error || articles.error ? (
        <section className="public-status">
          <h1>Não foi possível abrir este livro.</h1>
          <PrimaryButton onClick={() => { void books.mutate(); void articles.mutate() }}>Tentar novamente</PrimaryButton>
        </section>
      ) : null}
      {book ? (
        <>
          <header className="book-header">
            <p className="editorial-kicker">Caderno de leitura</p>
            <h1>{book.title}</h1>
            <p>{book.author}</p>
      ...
      {books.data && !book ? (
        <section className="public-status">
          ...
          <Link to="/livros">Voltar à estante</Link>
        </section>
      ) : null}
    </PublicReadingShell>
  )
