...
export function PublicArticlePage() {
  ...
  useDocumentTitle(`${article?.title ?? 'Artigo'} — deixedeserburro`)

  return (
    <PublicReadingShell>
      {isLoading ? <p className="public-status" role="status">Abrindo o ensaio…</p> : null}
      {error ? (
        <section className="public-status">
          <h1>Artigo não encontrado.</h1>
          <PrimaryButton onClick={() => void mutate()}>Tentar novamente</PrimaryButton>
          <Link to="/artigos">Voltar ao arquivo</Link>
        </section>
      ) : null}
      {article ? (
        <article className="prose public-article">
          <div className="article-meta">
            <Link to={`/livros/${article.sourceBook.slug}`}>{article.sourceBook.title}</Link>
            <span>{article.readingMinutes} min de leitura</span>
          </div>
          <Suspense fallback={<p role="status">Preparando a leitura…</p>}>
