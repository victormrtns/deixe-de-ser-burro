  ...
  return (
    <PublicReadingShell>
      <header className="archive-header">
        <p className="editorial-kicker">Arquivo público</p>
        <h1>Artigos</h1>
        <p>Ensaios nascidos de livros, notas e estudo.</p>
      </header>
      {isLoading ? <p className="public-status" role="status">Abrindo o arquivo…</p> : null}
      {error ? (
        <section className="public-status">
          <h2>Não foi possível abrir os artigos.</h2>
        ...
        <ol className="archive-list">
          {articles.map((article, index) => (
            <li key={article.slug}>
              ...
              <article>
                <p>{article.sourceBook.title} · {article.readingMinutes} min de leitura</p>
