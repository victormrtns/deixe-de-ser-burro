...
function SaveStatus() { const { state, actions } = useWorkspace(); const labels = { idle: 'Sem alterações', dirty: 'Alterado', saving: 'Salvando…', saved: 'Salvo', failed: 'Não foi possível salvar.', conflict: 'Versão mais recente disponível.' }; return <div className={`workspace-save workspace-save--${state.saveState}`} role="status"><span>{labels[state.saveState]}</span>{state.saveState === 'failed' ? <button type="button" onClick={() => void actions.retrySave()}>Tentar novamente</button> : null}</div> }
