...
export function LibraryPage() {
  ...
  async function signOut() {
    setSigningOut(true)
    try { await sessionActions.signOut(); await navigate('/') }
    catch { notify('Não foi possível sair agora. Tente novamente.', 'error'); setSigningOut(false) }
  }

  return <><header className="app-header"><span className="brand"><img src="/favicon.svg" alt="" width={30} height={30} /><img className="brand__wordmark" src="/brand/wordmark.svg" alt="deixedeserburro" width={176} height={30} /></span><div className="header-actions"><span>{session.status === 'author' ? session.email : ''}</span><GhostButton type="button" busy={signingOut} onClick={() => void signOut()}>Sair</GhostButton></div></header><main className="library" aria-label="deixedeserburro">
    ...
    <section><div className="section-title"><div><small>01</small><h2>Na estante</h2></div><NeutralButton onClick={() => setDialogOpen(true)} icon={<Plus size={16} />}>Adicionar livro</NeutralButton></div>
      {isLoading ? <div className="library-empty" role="status" aria-label="Carregando biblioteca">Abrindo a estante…</div> : null}
      ...
      {books?.length ? <div className="books-grid">{books.map((book, index) => <article className="book-card" key={book.id}><div className="book-cover"><small>{String(index + 1).padStart(2, '0')}</small><i /></div><div><h3>{book.title}</h3><p>{book.author}</p><small>{book.writingCount} {book.writingCount === 1 ? 'escrita' : 'escritas'}</small></div><GhostButton type="button" onClick={() => navigate(`/studio/livros/${book.id}`)} aria-label={`Abrir ${book.title}`}><ChevronRight size={18} aria-hidden="true" /></GhostButton></article>)}</div> : null}
    </section>
    <BookFormDialog open={dialogOpen} onOpenChange={setDialogOpen} onCreate={async (values, key) => { await api.books.create(values, key); await mutate(); notify(`“${values.title}” entrou na estante.`, 'success') }} />
  </main></>
}
