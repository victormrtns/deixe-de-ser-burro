...
it('renderiza somente artefatos públicos da escrita', async () => { render(<AppProviders api={createMockApi()}><MemoryRouter><PublicArticlePage /></MemoryRouter></AppProviders>); expect(await screen.findByRole('heading', { name: 'Ritual antes do foco' })).toBeVisible(); expect(screen.queryByText('Transcrição original')).not.toBeInTheDocument(); expect(screen.queryByRole('button', { name: 'Aceitar alteração' })).not.toBeInTheDocument() })
